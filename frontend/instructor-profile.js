...
async function loadProfile() {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/instructor/${instructorId}/profile`);
...
function setupPasswordForm() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/instructor/${instructorId}/password`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
...
document.addEventListener("DOMContentLoaded", () => {
  if (!requireRole("instructor")) return;
  setupLogoutHandlers();
  setupProfileForm();
  setupPasswordForm();
  loadProfile();
